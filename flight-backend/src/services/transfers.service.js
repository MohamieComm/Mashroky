...
export async function getTransferDetails(params = {}) {
  const { transferId } = params;
  if (!transferId) {
    const err = new Error('missing_transfer_id');
    err.status = 400;
    throw err;
  }
  const config = await resolveConfig();
  if (config.baseUrl) {
    try {
      ...
      const headers = await buildAuthHeaders(config);
      const res = await axios.get(url, { params: { id: transferId }, headers, timeout: 15000 });
      const item = res.data?.result || res.data || {};
      return normalizeTransferItem(item);
    } catch (e) {
      await logAmadeusError('transfer_details_error', { error: e?.message });
    }
  }
  return { id: transferId, name: null, message: 'details_from_cache' };
}
